...
describe("Tests", () => {
  ...
  var isAnagram = function(test, original) {
    ...
    for (let i = 0; i < original.length; i++) {
      if (freq.hasOwnProperty(original[i])) {
        freq[original[i]] -= 1;
        if (freq[original[i]] === 0) delete freq[original[i]];
        } else {
          return false
        }
    }
    return Object.keys(freq).length === 0 && freq.constructor === Object
  };